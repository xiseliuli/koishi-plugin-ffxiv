...
import { createSafeImageSegment } from './lib/canvas/Util';
    ...
    .action(async ({ session }, name) => {
      ...
      try {
        ...
        while (true) {
          ...
          if (!searchResult.Pagination.Results)
            return `没有找到名字中包含“${name}”的物品。`;
        ...
        while (!item) {
          if (itemList.length > 1) {
            const pageItemList = itemList.slice(
              (page - 1) * limit,
              page * limit
            );
            const textResult =
              ...
              pageItemList
                .map(
                  (item, index) =>
                    `${index + 1}. [${item.LevelItem}]${item.Name}`
                )
                .join('\r') +
              '\r' +
            ...
            if (
              Number.isNaN(replyInt) ||
              !replyInt ||
              replyInt < 1 ||
              replyInt > pageItemList.length
            )
              return;
        ...
        const timer = setTimeout(() => {
          session.send(
            wrapReply(session, '由于数据量较大，生成需要一段时间，请耐心等待……')
          );
        }, 5000);
        ...
        // 第一次调用用于计算尺寸
        await drawItemInfo(ctx, itemInfo, data, false, config);
        // 第二次调用生成最终图片
        const result = await drawItemInfo(ctx, itemInfo, data, true, config);
        clearTimeout(timer);

        // 使用安全的图片消息生成函数，确保图片完全生成
        const imageSegment = await createSafeImageSegment(
          result,
          config.saveToLocal
        );
