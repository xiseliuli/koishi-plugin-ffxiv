  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      const getTendency = (seed) => {
        ...
        else if (result === 6 || result === 8 || result === 16 || result === 18)
          return '吉';
        else return '小吉';
      };
      ...
      for (let i = 0; i < count && i < doings.length; i++) {
        let index =
          (result - i * Math.ceil((doings.length + 1) / count)) % doings.length;
        while (doingsResult.includes(doings[index]))
          index = index + 1 >= doings.length ? 0 : index + 1;
        doingsResult.push(doings[index]);
      }

      return (
        `今日抽到了【${getTendency(result)}】签！\r` +
