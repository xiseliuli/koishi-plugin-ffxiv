...
// 加载图像的缓存版本
async function loadImageCached(url: string, loadImageFn: any) {
...
let w = 0,
  h = 0;
export async function drawItemInfo(
  koishiCtx: Context,
  itemInfo: ItemInfo,
  data: Data,
  redraw: boolean = false,
  userConfig?: { saveToLocal?: boolean }
): Promise<{ buffer: Buffer; filePath?: string }> {
  const { Canvas, loadImage } = koishiCtx.skia;
  const config = getOptimizedConfig(userConfig);
  
  if (config.enablePerformanceLogging) {
    DrawingPerformance.start('drawItemInfo');
  }
  const top = 16,
    bottom = 16,
    left = 16,
  ...
  // 启用GPU渲染（如果可用）
  try {
    (canvas as any).gpu = true;
    
    // 检查渲染引擎状态
    const engine = (canvas as any).engine;
    if (engine && engine.error) {
      console.warn('GPU rendering failed, falling back to CPU:', engine.error);
    } else if (engine) {
      console.log(`Using ${engine.renderer} rendering with ${engine.api} on ${engine.device}`);
    }
  } catch (error) {
    console.warn('GPU configuration not available:', error);
  }
  ...
  // 并行加载所有需要的图像
  const [icon_hq, icon_gil] = await Promise.all([
    loadImageCached(path.join(__root_dir, '/public/image/hq.png'), loadImage),
    loadImageCached(path.join(__root_dir, '/public/image/gil.png'), loadImage)
  ]);

  const patchCategoryName = (() => {
    for (const i in data.patch.categoryIndex)
      if (i === itemInfo.item.patchCategory.toString())
        return data.patch.categoryIndex[i];
    return null;
  })();
  const categoryName = (() => {
    for (const i in data.item.categoryIndex)
      if (data.item.categoryIndex[i].id === itemInfo.item.category)
        return data.item.categoryIndex[i].name;
    return null;
  })();
  ...
  const findItemPartial = (id: string | number) => {
    for (const i in itemInfo.partials)
      if (
        itemInfo.partials[i].type === 'item' &&
        itemInfo.partials[i].id === id.toString()
      )
        return itemInfo.partials[i].obj;
    return null;
  };
  ...
  // 物品基本信息栏
  {
    const itemIcon = await loadImageCached(
      `https://garlandtools.cn/files/icons/item/${itemInfo.item.icon}.png`,
      loadImage
    );
    ...
    ctx.drawImage(
      ...
      drawPos[0],
      drawPos[1],
      (iconHeight / itemIcon.height) * itemIcon.width,
      iconHeight
    );
    ...
    const itemInfoText = `品级${itemInfo.item.ilvl} | ${
      patchCategoryName || '未知分类'
    } | ${categoryName || '未知种类'}`;
    const itemInfoTextMeasure = ctx.measureText(
      itemInfoText,
      drawWidth - (drawPos[0] - left)
    );
    ...
    ctx.textBaseline = 'top';
    ctx.textWrap = true;
    itemInfo.item.description = itemInfo.item.description.replace(
      /<br\s*\/?>/g,
      '\n'
    );
    const itemDescMeasure = ctx.measureText(
  ...
  // 制作
  {
    if (itemInfo.item.craft && itemInfo.item.craft.length) {
      ...
      const getIngredient = (id: number) => {
        ...
        for (const i in data.item.ingredients)
          if (data.item.ingredients[i].id === id)
            return data.item.ingredients[i];
        return null;
      };
      ...
      for (const craft of itemInfo.item.craft) {
        ...
        const ingredientTitleMeasure = ctx.measureText(
          ingredientTitle,
          drawWidth - (drawPos[0] - left)
        );
        ...
        const ingredientInfoMeasure = ctx.measureText(
          ingredientInfo,
          drawWidth - (drawPos[0] - left)
        );
        ...
        for (const craftItem of craft.ingredients) {
          ...
          const ingredient_icon = await loadImageCached(
            `https://garlandtools.cn/files/icons/item/${ingredient.icon}.png`,
            loadImage
          );
          ...
          ctx.drawImage(
            ...
            drawPos[0],
            drawPos[1],
            (ingredientNameHeight / ingredient_icon.height) *
              ingredient_icon.width,
            ingredientNameHeight
          );
          drawPos[0] +=
            (ingredientNameHeight / ingredient_icon.height) *
              ingredient_icon.width +
          ...
          ctx.fillText(
            ingredientName,
            drawPos[0],
            drawPos[1] + ingredientNameHeight / 2
          );
  ...
  // 来源：商店购买
  {
    if (itemInfo.item.vendors && itemInfo.item.vendors.length) {
      ...
      ctx.drawImage(
        ...
        drawPos[0],
        drawPos[1],
        (priceTitleHeight / icon_gil.height) * icon_gil.width,
        priceTitleHeight
      );
  ...
  // 来源：雇员探险
  {
    if (itemInfo.item.ventures && itemInfo.item.ventures.length) {
      ...
      for (const v of itemInfo.item.ventures) {
        ...
        for (
          let i = 0;
          i <= 2 && venture.lvl + i * 10 <= data.xp.length - 1;
          i++
        ) {
      ...
      for (const s of itemInfo.item.tradeShops) {
        ...
        tradeShopNpc.forEach((n) => {
          ...
          const area = n.a
            ...
            : { name: '未知区域' };
          const zone = n.l
            ? getLocationIndex(n.l as number)
            : { name: '未知地图' };
          const locationText =
            `${zone.name} ${area.name}` +
            (n.c ? `(${(n.c as number[]).join(',')})` : '');
        ...
        for (const l of s.listings) {
          ...
          for (const i of l.item) {
            ctx.save();
            const listItem =
              i.id === itemInfo.item.id.toString()
                ? {
                    i: itemInfo.item.id,
            ...
            ctx.drawImage(
              ...
              drawPos[0],
              drawPos[1],
              (tinyIconHeight / icon.height) * icon.width,
              tinyIconHeight
            );
            drawPos[0] +=
            ...
            if (i.hq) {
              ctx.drawImage(
                icon_hq,
                drawPos[0],
                drawPos[1] - iNameTextHeight / 2,
                (icon_hq.height / iNameTextHeight) * icon_hq.width,
                iNameTextHeight
              );
          ...
          for (const c of l.currency) {
            ...
            ctx.save();
            const listItem =
              c.id === itemInfo.item.id.toString()
                ? {
                    i: itemInfo.item.id,
                    c: itemInfo.item.icon,
                    n: itemInfo.item.name,
            ...
            if (c.hq) {
              const iconWidth =
                (icon_hq.height / cNameTextHeight) * icon_hq.width;
              ctx.drawImage(
                icon_hq,
                drawPos[0] - iconWidth,
                drawPos[1] - cNameTextHeight / 2,
                iconWidth,
                cNameTextHeight
              );
              drawPos[0] -= iconWidth;
  ...
  // 用于制作
  {
    if (itemInfo.item.ingredient_of) {
      ...
      for (const id in itemInfo.item.ingredient_of) {
        ...
        const target_item_icon = await loadImageCached(
          `https://garlandtools.cn/files/icons/item/${target_icon_partial.c}.png`,
          loadImage
        );
        ...
        ctx.drawImage(
          ...
          drawPos[0],
          drawPos[1],
          (targetItemNameHeight / target_item_icon.height) *
            target_item_icon.width,
          targetItemNameHeight
        );
        drawPos[0] +=
          (targetItemNameHeight / target_item_icon.height) *
            target_item_icon.width +
  ...
  // 绘制背景
  drawBackground(ctx, canvas.width, canvas.height, '#182927');
  ...
  try {
    // 使用异步导出以获得更好的性能
    const buffer = await canvas.toBuffer(exportOptions.format, exportOptions);
    
    // 根据配置决定是否保存图片到本地
    const filePath = config.saveToLocal ? await ImageStorage.saveImage(buffer, exportOptions.format) : null;
    
    if (config.enablePerformanceLogging) {
      DrawingPerformance.end('drawItemInfo');
    }
    
    return { buffer, filePath: filePath || undefined };
  } catch (error) {
    console.error('Canvas export failed:', error);
    // 回退到简单导出
    const buffer = await canvas.toBuffer('png');
    const filePath = config.saveToLocal ? await ImageStorage.saveImage(buffer, 'png') : null;
