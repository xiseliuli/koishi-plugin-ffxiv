...
export function apply(ctx: Context) {
  const getRollResult = (min, max): string =>
    `抛出命运的骰子！您扔出了${
    ...
    .action(async ({ session }, ...args) => {
      if (args.length === 1) {
        ...
        let min = parseInt(args[0]),
          max = parseInt(args[1]);
        if (Number.isNaN(min) || Number.isNaN(max) || min === max || min < 1)
          return;
        if (min < max) {
          const t = min;
          min = max;
  ...
  ctx.middleware(async (session, next) => {
    try {
      if (session.content.match(/^[!！](r|random)(\s)*([0-9]+)?(d[0-9]+)?$/i)) {
        const regExp = session.content.match(
          /^[!！](r|random)(\s)*([0-9]+)?(d[0-9]+)?$/i
        );
        const diceCount = regExp[3] ? Number.parseInt(regExp[3]) : 1;
        const sideCount = regExp[4]
          ? Number.parseInt(regExp[4].substring(1))
          : 100;

        if (
          `${diceCount}${sideCount}` == '114514' ||
        ...
        await session.send(
          `当然是${
            parsedArgs[Math.floor(parsedArgs.length * Math.random())]
          }了。`
        );
        return;
      }
